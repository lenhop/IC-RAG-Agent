...
from ..memory.short_term import MemoryEvent
...
def _append_memory_event(
    *,
    ...
    """Best-effort memory event write to Redis (+ optional ClickHouse)."""
    if not gateway_memory or not user_id or not str(user_id).strip():
        return
    try:
...
async def rewrite(
    ...
    if rewritten_query and rewritten_query.strip():
        try:
            from ..route_llm.classification import split_intents, get_keyword_vector_results, resolve_intent
