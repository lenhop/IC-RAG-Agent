#!/usr/bin/env python3
"""
Load phrases from phrases_from_titles.csv into Chroma for intent classification.

Reads the phrase column only (first column, header skipped). One row = one chunk.
Uses same embedding model as RAG pipeline (minilm/ollama/qwen3) for consistency.

Flow:
  1. Load phrases from CSV
  2. Embed with local model
  3. Store in Chroma collection keyword
"""
...
DEFAULT_CSV_PATH = str(
    PROJECT_ROOT / "data" / "intent_classification" / "keywords" / "phrases_from_titles.csv"
)
CHROMA_PATH = _resolve_path(
    "CHROMA_KEYWORD_PATH", str(PROJECT_ROOT / "data" / "chroma_db" / "keyword")
)
...
def main() -> int:
    """Load keyword phrases to Chroma."""
    import argparse

    parser = argparse.ArgumentParser(
        description="Load phrases from phrases_from_titles.csv into Chroma"
    )
    parser.add_argument(
        "--csv-path",
        default=DEFAULT_CSV_PATH,
        help=f"Path to phrases CSV (default: {DEFAULT_CSV_PATH})",
    )
    parser.add_argument(
        "--chroma-path",
        default=CHROMA_PATH,
    ...
    # Load phrases (one row = one chunk, skip header)
    ...
    try:
        ...
        return 1
    if not phrases:
        print("[WARN] No phrases found in CSV")
        return 1

    print(f"[1/3] Loaded {len(phrases)} phrases from {csv_path}")
    ...
    for start in range(0, len(phrases), args.batch_size):
        end = min(start + args.batch_size, len(phrases))
