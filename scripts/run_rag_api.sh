...
# Phase 2: Concurrency and reliability for 10+ users.
...
# Load RAG API vars from .env (optional; api.py also loads .env)
if [[ -f .env ]]; then
...
# Phase 2: Uvicorn concurrency - allow 20 connections (10 users + buffer)
...
exec python -m uvicorn src.rag.api:app \
  --host "$HOST" \
  --port "$PORT" \
