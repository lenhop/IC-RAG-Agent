...
Usage:
    python scripts/uds/seed_data.py --rows 100000 --host localhost
...
CATEGORIES = ["Electronics", "Home & Kitchen", "Sports & Outdoors", "Toys & Games",
               "Health & Personal Care", "Beauty", "Office Products", "Pet Supplies"]
...
TRANSACTION_TYPES = ["Order", "Refund", "FBAFee", "ReferralFee", "StorageFee", "Adjustment"]
...
def generate_orders_and_items(products: list[dict], n_orders: int = 100_000):
    orders = []
    order_items = []
...
def insert_batch(client, table: str, rows: list[dict], batch_size: int = 10_000):
    if not rows:
        return
...
def main():
    ...
    parser.add_argument("--orders", type=int, default=100_000, help="Number of orders")
