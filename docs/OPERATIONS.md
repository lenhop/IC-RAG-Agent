...
- **GATEWAY_TEXT_GENERATION_BACKEND** (`deepseek` \| `ollama`): Selects the LLM for (1) RAG `amazon_business` merge after Chroma + evidence retrieval, and (2) optional gateway formatting of `sp_api` worker answers. Resolution matches other chat stages: `GATEWAY_TEXT_GENERATION_BACKEND` → `GATEWAY_CHAT_LLM_BACKEND` → default `deepseek`. At runtime, `complete_chat` may still fall back from DeepSeek to Ollama if DeepSeek is unavailable. The **RAG service (8002)** and **gateway (8000)** must both load the same `.env` if you expect matching behavior.
